...
void send_command(int qid, command cmd, char *msg) {
    ...
    msgbuf mbuf;
    mbuf.mtype = cmd;
    memset(mbuf.mtext, '\0', QMSG_LEN);
    snprintf(mbuf.mtext, QMSG_LEN, "%s", msg);
    if (msgsnd(qid, &mbuf, QMSG_LEN, 0) == -1) {
        error_and_end_errno();
    }
}

ssize_t receive_command(int qid, msgbuf *mbuf, long msgtype) {
    ssize_t received = msgrcv(qid, mbuf, QMSG_LEN, msgtype, 0);
    if (received == -1) {
...
ssize_t receive_command_noblock(int qid, msgbuf *mbuf, long msgtype) {
    ...
    return msgrcv(qid, mbuf, QMSG_LEN, msgtype, MSG_NOERROR | IPC_NOWAIT);
}
