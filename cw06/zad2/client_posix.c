...
static void sig_handler(int signo) {
    if (signo == SIGINT) {
        ...
        // SIGUSR1 indicates messages in the queue
        char response[QMSG_LEN];
        while (pos_receive_command_noblock(client_qid, response) != -1) {
...
void await_client_id(int client_qid) {
    char mbuf[QMSG_LEN];
    memset(mbuf, '\0', QMSG_LEN);
    pos_receive_command(client_qid, mbuf);
    if (*mbuf == '\0') {
        return;             // Received SIGINT while waiting for response, likely means there are too many clients
    }
    command type = strtol(strtok(mbuf, " "), NULL, 10);
    if (type != ACK) {
        error_and_end("Didn't get ACK message\n");
    }
...
int main() {
    ...
    srv_qid = mq_open(srv_queue_path, O_WRONLY);
    if (srv_qid == -1) {
        fprintf(stderr, "No server queue\n");
        error_and_end_errno();
    }
    ...
    client_qid = mq_open(client_queue_path, O_CREAT | O_RDONLY, 0666, &queue_attribs);
    if (client_qid == -1) {
    ...
    pos_send_command(srv_qid, INIT, init_msg);
    free(init_msg);
    ...
    // Inform server about client exit (if connection was established)
    ...
    if (client_id) {
        ...
        pos_send_command(srv_qid, STOP, exit_msg);
        free(exit_msg);
        usleep(300);        // give server time to close queue
    }
